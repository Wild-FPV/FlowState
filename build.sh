...
steamworksDestination="$buildFolder/steamworks"

#windows files
steamFiles=( "debug.bat" "steam_appid.txt" "steam_api64.dll" "steam_api64.lib" "steam_api.dll" "steam_api.lib" "SteamworksPy64.dll" "SteamworksPy.dll")
...
echo "copying steamworks"
cp -r steamworks $steamworksDestination
...
#copy all the steam integration files
for i in "${steamFiles[@]}"
do
   echo "copying $i to $buildFolder"
   cp $i $buildFolder
done
